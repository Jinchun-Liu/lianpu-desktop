...
class TpmDevice {
  ...
  _parse(stdout){let result;try{result=JSON.parse(String(stdout).trim());}catch{throw new DeviceError('DEVICE_RESPONSE','本机设备保护未返回有效结果。');}if(!result||typeof result.state!=='string')throw new DeviceError('DEVICE_RESPONSE','本机设备保护结果无效。');return result;}
  ...
  async invoke(command,bytes){this._available();return new Promise((resolve,reject)=>{const child=execFile(this.helper,[command],{windowsHide:true,timeout:20000,maxBuffer:65536},(error,stdout)=>{try{if(error&&(error.killed||error.signal))throw new DeviceError('DEVICE_OPERATION_UNCONFIRMED','本机设备操作未返回确认结果。请保留诊断，勿重复准备。','error');const result=this._parse(stdout);if(error&&(result.ok||!result.code))throw new DeviceError('DEVICE_FAILED','本机设备保护操作未完成。');resolve(result);}catch(e){reject(e);}});child.stdin?.end(bytes?Buffer.from(bytes).toString('base64url'):'');});}
  ...
  async _elevate(){
    let channel;try{
      ...
      const error=await new Promise(resolve=>{const child=execFile(path.join(system,'System32','WindowsPowerShell','v1.0','powershell.exe'),['-NoProfile','-NonInteractive','-Command',preparationScript(this.helper,this.expectedSha256,channel.name)],{windowsHide:true,timeout:120000,maxBuffer:65536},error=>resolve(error));child.stdin?.end();});
  ...
  signSync(bytes){this._available();const response=spawnSync(this.helper,['sign'],{windowsHide:true,timeout:20000,maxBuffer:65536,encoding:'utf8',input:Buffer.from(bytes).toString('base64url')});if(response.error||response.signal||response.status!==0)throw new DeviceError('DEVICE_SIGN_FAILED','设备签名进程未确认成功，请保留诊断信息。','error');const result=this._parse(response.stdout);if(!result.ok||result.state!=='ready'||result.deviceId!==this.info?.deviceId||result.publicKeySpki!==this.info?.publicKeySpki||result.signatureFormat!=='ieee-p1363'||result.hashAlgorithm!=='SHA-256')throw new DeviceError(result.code||'DEVICE_SIGN_FAILED',result.message||'设备签名未完成，请检查本机准备状态。',result.state);const signature=Buffer.from(result.signature||'','base64url');if(signature.length!==64||signature.toString('base64url')!==result.signature||!this.verify(bytes,signature))throw new DeviceError('DEVICE_SIGNATURE','设备签名不能通过本机核验。');return signature;}
